...
- **Windows-side WezTerm config is a manual copy, not synced.** On WSL2 machines, WezTerm runs from Windows and reads `%USERPROFILE%\.config\wezterm\wezterm.lua` (visible from WSL at `/mnt/c/Users/<winuser>/.config/wezterm/wezterm.lua`). Currently kept in sync by hand — edits to `wezterm/wezterm.lua` in this repo must be mirrored there. TODO: explore extending `install.sh` to deploy the Windows-side copy automatically. Constraints: (a) must detect WSL2 vs native Linux (`/proc/version` grep for `microsoft`, or `$WSL_DISTRO_NAME` set), (b) Windows username is not `$USER` — resolve via `/mnt/c/Users/` enumeration or `cmd.exe /c echo %USERNAME%` (latter is slow and noisy), (c) decide symlink vs copy — symlinks across `/mnt/c` work but WezTerm on Windows may not follow them reliably, copy is safer but drifts, (d) must no-op cleanly on machines without `/mnt/c` (native Linux, macOS), (e) need a backup path matching the rest of `install.sh`'s `.backup-<timestamp>` convention.
