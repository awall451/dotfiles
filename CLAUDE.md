...
- `~/.gitconfig-work` — loaded **only** for Azure DevOps remotes via `[includeIf "hasconfig:remote.*.url:**dev.azure.com**"]` and `**visualstudio.com**` blocks at the bottom of `git/gitconfig`. Untracked. Holds the work `[user]` block on the work machine; absent on personal machines (the `includeIf` no-ops). The tracked `[user]` block sets a personal email as the default; Azure remotes override via this file. Never write the employer name in tracked files — keep work-routing references generic ("work identity", "Azure DevOps remotes").
